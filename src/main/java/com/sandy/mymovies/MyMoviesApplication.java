...
public class MyMoviesApplication implements CommandLineRunner {
    ...
    private void testRepos() {
        ...
        final Optional<Chapter> chapter = chapterRepository.findByImdbIdAndSeasonAndEpisodeNumber("4052886",1,1);
        if (chapter.isPresent()) {
            log.info("Found: " + chapter.get().toString());
        } else {
            log.info("Unable to load episode with id: " + "4052886" + " season " + "1" + " episode "+"1");
        }
    }
