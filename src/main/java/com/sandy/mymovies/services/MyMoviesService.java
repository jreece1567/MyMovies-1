...
public class MyMoviesService {
  ...
  public Movie createMovie(final Movie movie) {
    ...
    if (!video.isPresent()) {
      ...
      // save the Video
      videoRepository.save(new Video(movie.getImdbId(), movie.getTitle(), movie.getReleaseYear(),
          movie.getDuration(), movie.getRating(), movie.getDirector(), movie.getImageUrl(),
  ...
  public Title readTitle(final String imdbId) {
    ...
    return new Title(video.get().getImdbId(), video.get().getTitle(), video.get().getReleaseYear(),
